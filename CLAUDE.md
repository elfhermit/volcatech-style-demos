...
9. **0814 盤點抓到的其餘修正**(來源 `docs/reports/改善盤點_20260814.md`,51 條成立發現):
   ...
   **同日晚間隨第 10 條改回 `Three`**)、
   ...
   前面那個箭頭指向空白;⚠ **這條當時沒有實際渲染驗證,寫下去的規則其實是壞的,見 0815 第三輪**)、
   `All ai services` → `All AI services`、`imprint` 版權行的 `Ltd..`、
...
**2026-08-15 第三輪(`.steps` 流程圖跑版修復)**——使用者回報 `managed-gcp.html` 的
六階段流程圖線上跑版。實測(headless Chrome 量測每個 `li` 的實際盒子)確認三件事:

1. **`.steps.six` 從第一天就是壞的**。`.steps>li.ar{flex:0 0 auto}` 與
   `.steps.six>li{flex:1 1 calc(33.333% - 46px)}` **特異度同分**(皆 0,2,1),後寫的贏,
   於是每個箭頭都拿到一整格卡片寬,整段排成「卡片／箭頭／卡片」交錯的三欄空格陣。
   修法是把修飾類寫成 `>li:not(.ar)`,讓它根本選不到箭頭,不靠原始碼順序決勝。
   現況:≥901px 乾淨的三＋三,每列卡片等寬、列首無懸空箭頭。
2. **同一個病在別頁也發作,只是門檻不同**。`flex-basis` 是「何時換行」的門檻,不是寬度
   ——卡片同時帶 `flex-grow`,一列放得下時實際寬度只由 `(列寬 − 箭頭 − gap) ÷ 卡片數`
   決定,**調 basis 不會改變任何已經放得下的畫面**。因此 basis 由 200px 降為 **170px**,
   四格流程(6 頁)在 901–940px 不再散成「三格＋懸空箭頭＋第四格獨佔一列」。
3. **`ess.html` 五格流程新增 `.five` 修飾類**。它在 901–1180px 一直是
   「四格＋懸空箭頭／第五格獨佔一列」——正是 `.six` 當初被建出來要避免的缺陷。
   ≥1200px 維持一列五格(與原本逐像素相同),以下走 `.six` 的三＋二。

⚠ 這一輪的教訓已登記在全域教訓簿。**驗證方式不變,但要知道 `grep` 驗不到它**:
三支腳本與所有內容檢查在壞掉的那三天全程 PASS——版面是否成立只能實際渲染來看,
所以改 `BLOCK_CSS` 的版面規則之後,請用 headless 量測或截圖看過再宣稱完成。
...
| `.steps` | **有先後順序**的流程 | `#111A26` 節點 ＋ `--muted` 邊框 ＋ 節點間的真箭頭。**卡片數決定修飾類**:2–4 項不加;5 項加 `.five`(≥1200px 一列五格,以下退三＋二);6 項加 `.six`(三＋三)。⚠ `flex-basis` 是「何時換行」的門檻不是寬度(卡片同時 `flex-grow`,一列放得下時實際寬度只由列寬與卡片數決定),0815 由 200px 調為 170px 是為了讓四格在 901–940px 不散開 |
